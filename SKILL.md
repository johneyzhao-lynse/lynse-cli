...
## ✨ Friendly Commands (v1.4.0)

Legacy API-style commands (`getCurrentCustomer`, `listFilesByTimeRange`, ...) remain fully supported.
New friendly aliases are available for human and script use:
...
lynse meetings outline <id>           # Get outline
lynse meetings info <id>              # Meeting details
lynse folders list                    # List folders/groups
lynse folders create <json>           # Create folder
lynse folders move <json>             # Move files to folder
lynse todos list [all|open|done]      # List todos
lynse todos delete <ids>              # Delete todos
...
Combine flags: `lynse meetings list --table --output meetings.txt`

### Auth Commands

```
lynse auth login --api-key <key> [--host <url>]   # Save & validate API key
lynse auth status                                  # Show auth config
lynse auth logout [--all]                          # Clear tokens (and API key with --all)
lynse auth doctor                                  # Diagnose auth issues
```

### System Commands

```
lynse version    # Show version, Python, OS, requests info
lynse doctor     # Full environment diagnostics
lynse update     # Show update instructions
```

All system commands support `--text` for human-readable output.
...
| Scope | 说明 | 典型操作 |
|-------|------|----------|
| `customer.read` | 读取用户信息 | getCurrentCustomer, getUserInfo, refreshMembership |
...
| `device.read` | 读取设备信息 | getMyDevices, getDeviceInfo, getDevicePage |
...
python lynse.py refreshMembership           # 当前会员等级与额度
python lynse.py getUserInfo <用户 ID>        # 指定用户信息
python lynse.py getCurrentUser              # 当前系统用户
```

当用户问“我当前是什么会员等级”“当前会员”“会员等级”这类查询本人账户状态的问题时，调用 `refreshMembership`，从返回的 `data.memberLevel` 回答；不要把这类问题当作会员规则说明处理。
...
### 🔹 设备管理
```bash
python lynse.py getMyDevices                # 当前绑定设备列表（含 serialNumber/authSn）
python lynse.py getDevicePage <页码>      # 分页设备列表
python lynse.py getDeviceInfo <设备 ID>    # 设备详情
python lynse.py unbindDevice <设备 ID>     # 解绑设备
```

当用户问“绑定设备”“设备 SN”“我的设备 SN 号”时，必须优先调用 `getMyDevices`，接口为 `GET /api/business/device/mine`。回答 SN 时优先使用 `serialNumber`，没有时再使用 `authSn`，不要用旧的 `getDevicePage` 猜测。
...
### 🔹 认证管理 (v1.4.0)
```bash
python lynse.py auth login --api-key <key> --host <url>   # 保存并验证 API Key
python lynse.py auth status                               # 查看当前认证配置
python lynse.py auth logout [--all]                       # 清除缓存 token（--all 同时清除 API Key）
python lynse.py auth doctor                               # 诊断认证问题
```
...
### 🔹 诊断 (v1.4.0)
```bash
python lynse.py version                  # 版本信息（lynse-cli / Python / OS / requests）
python lynse.py doctor                   # 全面环境诊断（Python版本/依赖/网络/认证/用户）
python lynse.py update                   # 查看更新说明
```
...
用户调用 → lynse.py
  → 检查 LYNSE_API_HOST / LYNSE_API_KEY
    → 不存在 → 检查 ~/.lynse/config.json
      → 不存在 → 检查 .env
        → 不存在 → 提示配置
  → 存在 → 检查缓存 Token (~/.lynse/tokens.json 或 .token_cache)
    → Token 有效 → 直接使用
...
**配置解析顺序 (v1.4.0)：**
1. 命令行参数 `--api-key` / `--host`
2. 环境变量 `LYNSE_API_KEY` / `LYNSE_API_HOST`
3. 用户级配置 `~/.lynse/config.json`
4. 项目级 `.env` 文件（向后兼容）
...
- ✅ 新增输出格式控制（`--json`, `--pretty`, `--text`, `--table`, `--output`）
- ✅ 新增语义化退出码（0-6，取代直接传递 HTTP 状态码）
- ✅ 新增 `auth login/status/logout/doctor` 认证管理命令
- ✅ 新增 `version` / `doctor` / `update` 系统诊断命令
- ✅ 改进配置路径：支持 `~/.lynse/config.json` 用户级配置
- ✅ Token 缓存默认路径迁移到 `~/.lynse/tokens.json`
- ✅ 保留原有 API 风格命令完全向后兼容
